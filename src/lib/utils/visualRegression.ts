    ...
    const onImageLoad = () => {
      ...
      if (loadedCount === 2) {
        try {
          ...
          resolve({ similarity, ...(difference && { difference }) });
        } catch (error) {
          reject(error);
        }
      }
...
const calculatePixelSimilarity = (
  ...
  for (let i = 0; i < data1.length; i += 4) {
    ...
    const a1 = data1[i + 3] ?? 0;

    const r2 = data2[i] ?? 0;
    const g2 = data2[i + 1] ?? 0;
    const b2 = data2[i + 2] ?? 0;
    const a2 = data2[i + 3] ?? 0;
...
const generateDifferenceImage = (
  ...
  for (let i = 0; i < data1.data.length; i += 4) {
    const r1 = data1.data[i] ?? 0;
    const g1 = data1.data[i + 1] ?? 0;
    const b1 = data1.data[i + 2] ?? 0;
    const a1 = data1.data[i + 3] ?? 0;

    const r2 = data2.data[i] ?? 0;
    const g2 = data2.data[i + 1] ?? 0;
    const b2 = data2.data[i + 2] ?? 0;
    const a2 = data2.data[i + 3] ?? 0;
    ...
    for (const section of testConfig.sections) {
      try {
        ...
        const result: ScreenshotComparison = {
          ...
          ...(comparison.difference && { difference: comparison.difference }),
